...
def get_tables(conn):
    try:
        ...
        messagebox.showerror("Ошибка получения таблиц", "Не удалось получить список таблиц. Пожалуйста, попробуйте позже.")
        return []
...
def get_table_data(conn, table_name):
    try:
        ...
        messagebox.showerror("Ошибка получения данных", f"Не удалось получить данные из таблицы {table_name}. Пожалуйста, попробуйте позже.")
        return [], []
...
def search_by_teacher(conn, teacher_name):
    try:
        with conn.cursor() as cursor:
            ...
            """, (f"%{teacher_name}%",))
            rows = cursor.fetchall()
            column_names = ["Номер преподавателя", "ФИО преподавателя", "Дисциплина", "День", "Пара", "Группа", "Номер кабинета"]
        return column_names, rows
    except Exception as e:
        messagebox.showerror("Ошибка поиска", "Не удалось найти информацию о преподавателе. Пожалуйста, проверьте введенные данные и попробуйте снова.")
        return [], []
...
def search_by_group(conn, group_name):
    try:
        with conn.cursor() as cursor:
            ...
            """, (f"%{group_name}%",))
            rows = cursor.fetchall()
            column_names = ["Группа", "Количество человек", "День", "Пара", "Дисциплина", "ФИО преподавателя", "Номер кабинета"]
        return column_names, rows
    except Exception as e:
        messagebox.showerror("Ошибка поиска", "Не удалось найти информацию о группе. Пожалуйста, проверьте введенные данные и попробуйте снова.")
        return [], []
...
def check_conflicts(conn, day, pair, group=None, teacher=None, room=None, current_id=None):
    try:
        ...
        messagebox.showerror("Ошибка проверки конфликтов", "Не удалось проверить конфликты. Пожалуйста, попробуйте позже.")
        return False
...
class App(tk.Tk):
    ...
    def edit_record(self):
        ...
        if dialog.result:
            ...
            group_index = self.current_tree['columns'].index('Группа') if 'Группа' in self.current_tree['columns'] else None
            teacher_index = self.current_tree['columns'].index('Номер_преподавателя') if 'Номер_преподавателя' in self.current_tree['columns'] else None
            room_index = self.current_tree['columns'].index('Номер_кабинета') if 'Номер_кабинета' in self.current_tree['columns'] else None
            ...
            if day and pair and (group or teacher or room):
                if check_conflicts(self.conn, day, pair, group, teacher, room, primary_key):
                    messagebox.showerror("Ошибка редактирования записи", "Произошел конфликт расписания. Пожалуйста, измените параметры.")
                    return
            ...
            try:
                ...
                messagebox.showerror("Ошибка редактирования записи", "Не удалось обновить запись. Пожалуйста, проверьте введенные данные и попробуйте снова.")
                return
    ...
    def add_record(self):
        ...
        if dialog.result:
            if any(value == '' for value in dialog.result):
                messagebox.showerror("Ошибка добавления записи", "Не все поля заполнены. Пожалуйста, заполните все поля и попробуйте снова.")
                return
            ...
            if day and pair and (group or teacher or room):
                if check_conflicts(self.conn, day, pair, group, teacher, room):
                    messagebox.showerror("Ошибка добавления записи", "Произошел конфликт расписания. Пожалуйста, измените параметры.")
                    return
            ...
            try:
                ...
                messagebox.showerror("Ошибка добавления записи", "Не удалось добавить запись. Пожалуйста, проверьте введенные данные и попробуйте снова.")
                return
